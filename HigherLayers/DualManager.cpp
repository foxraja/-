...
{	// µà¾ó Å¬·¡½º·Î º¯È¯ µÇ¾ú´Ù´Â Packet¸¦ ¹ÞÀ½
	CHARLIST* pMaster = ::CheckServerId(idMaster);
	if (pMaster == NULL)  return;
	// µà¾ó ¿©ºÎ °Ë»ç
	if (pMaster->IsDual())
...
{	// µà¾ó Å¬·¡½º ¾îºô¸®Æ¼ ºÐ¹è¸¦ ÇÏ°Ú´Ù´Â Packet¸¦ ¹ÞÀ½
	CHARLIST* pMaster = ::CheckServerId(idMaster);
	if (pMaster == NULL)  return;
...
void CDualManager::RecvResetAbility(WORD idMaster)   //101ÖØ·ÖµãÏÞÖÆ
{
	CHARLIST* pMaster = ::CheckServerId(idMaster);
	if (pMaster == NULL)  return;
	
//	if (pMaster->GetLevel() == CROSSING_CLASS_LEVEL)   ×¢ÊÍµô
//	{	//< CSD-030806   ×¢ÊÍµô
...
{	//< CSD-TW-030606 : µà¾ó Å¬·¡½º ´Ü°è »ó½Â¿¡ µû¸¥ ¼Ò¸ð Ladder³ª Fame
