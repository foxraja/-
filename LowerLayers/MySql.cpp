...
//				ÀÐ¾î¿Â´Ù)¸¦ ÀúÀå(update)ÇÏ´Â ÇÔ¼ö.
//
...
//
// ¸®ÅÏ°ª: updateÀÇ ¼º°ø ¿©ºÎ; ¼º°ø(1), ½ÇÆÐ(ret < 0)
//
/////////////////////////////////////////////////////////////////////////////
...
{		
	...
	sprintf(query, "UPDATE chr_info SET"
		...
		chr->GetBasicResist(RT_POISON), // ÀúÁÖ°è¿­ ¸¶¹ý¿¡ ´ëÇÑ ÀúÇ×·Â
		chr->GetBasicResist(RT_CURSE),	 //	ÀúÁÖ°è¿­ ¸¶¹ý¿¡ ´ëÇÑ ÀúÇ×·Â
		chr->GetBasicResist(RT_HOLY),    // ½Å·Â°è¿­ °ø°Ý ¸¶¹ý¿¡ ´ëÇÑ ÀúÇ×·Â
		chr->GetBasicResist(RT_FIRE),     //	ºÒ°è¿­ °ø°Ý ¸¶¹ý¿¡ ´ëÇÑ ÀúÇ×·Â
		chr->GetBasicResist(RT_ICE) ,      //	¾óÀ½°è¿­ °ø°Ý ¸¶¹ý¿¡ ´ëÇÑ ÀúÇ×·Â
		chr->GetBasicResist(RT_ELECT),   //	Àü°Ý°è¿­ °ø°Ý ¸¶¹ý¿¡ ´ëÇÑ ÀúÇ×·Â
		chr->X, chr->Y,
...
void updateCharacterStatusToLoginServer( t_connection c[], int char_id)
{
	...
	tp->Sight = ch->Sight;
	tp->Age = ch->Age;
	tp->bAlive		= ch->bAlive;					// Ä³¸¯ÅÍÀÇ »ý»ç¿©ºÎ(	REMOVE_:0 / ALIVE_:1 / DEAD_:2 / BUFE_:3)
	...
	CheckFightMap(tp->MapName, tp->X, tp->Y, ch );	
...
int InsertBBS( char *name, char *title, char *contents )
{
	...
	// Ä³¸¯ÅÍÀÇ ÀÏ¹Ý Data¸¦ chr_info¿¡ »ý¼ºÇÏ°í ( 1-1´Ü°è )
	SQLAllocStmt(hDBC, &hStmt);
	retCode = SQLExecDirect(hStmt, (UCHAR *)szQuerry, SQL_NTS);
	if(retCode == SQL_SUCCESS || retCode == SQL_SUCCESS_WITH_INFO)
	{
...
int UpdateEvaName( char *my_name, char *eva_name )		// ÀüµµÇÑ »ç¶÷ ÀÌ¸§À» µðºñ¿¡ ¾÷µ¥ÀÌÆ® ÇÑ´Ù. // 0405 YGI
{
	HSTMT		hStmt = NULL;
	RETCODE		retCode;
	char		szQuerry[255];
...
{	
	...
	//	for(int i=0;i<MaxGenSpcItem;i++)
	{
		...
		sprintf(query, "UPDATE Special_item SET Now_Store = %d , S_Day = %d where No= '%d'" , GenSpcItem[i].now_store,
...
int CheckSpecialItemRate(int i,int How)
{
	if( rand()%GenSpcItem[i].howmany != 0 )								//ÇØ´çÇÏ´Â È®·üÀÌ¾Æ´Ï¸é
		return -1;
	if( How!=GenSpcItem[i].howget && GenSpcItem[i].howget!=0 )		//¸¸¾à ¾ò´Â ¹æ¹ýÀÌ ÀÖÀ¸¸é ¶Ç´Â »ó°ü ¾øÀ¸¸é
		return -2;
	return 1;
}
...
int GiveSpecialItem(int NPCno,int dx,int dy,int MapNo,int How)
{
	...
	for(int i=0;i<MaxGenSpcItem;i++)
	{
		...
		UpdateSpecialItem( i );							//¾ÆÀÌÅÛÀ» »ý¼ºÇß´Ù±¸ µðºñ¿¡ ¾´´Ù.
...
{			
	...
	sprintf(szQuerry, "SELECT * FROM Special_Item ORDER BY no" );
...
{//021113 lsw
	...
	{			
		...
		{	
			...
			strupr( map[count].mapfile );			// 1027 YGI
...
int Get_INT_ValueBySQL( const char *table, const char *field, char *condition, int *value, int max )		
{
	...
	if( condition )
		sprintf(szQuerry, "SELECT %s FROM %s", field, table);
	else	sprintf(szQuerry, "SELECT %s FROM %s WHERE %s", field, table, condition );
...
int InitGmQuest( void )
{
	...
	strcpy(query_stmt, "select * from GMquest") ;
	ret= SQLExecDirect(hStmt, (UCHAR *)query_stmt, SQL_NTS) ;
...
int CAliveMapZoneAll::LoadAliveMapZone()
{
	...
	strcpy(query_stmt, "SELECT map_number, x, y, xl, yl, is_all, alive_word_type FROM alive_zone") ;
	ret= SQLExecDirect(hStmt, (UCHAR *)query_stmt, SQL_NTS) ;
	if(ret != SQL_SUCCESS_WITH_INFO && ret != SQL_SUCCESS) 
	{
...
int LoadNationInfo( tagNationInfo *NationInfo )
{
	...
	sprintf(szQuerry, "SELECT nation_code, nation_money, main_map_port, visa_va, visa_zy, visa_ill, "
...
int LoadNationInfoOfMainMap( int nation )		// 010528 YGI
{
	...
	sprintf(szQuerry, "SELECT king_support, friend_va, friend_zy, friend_ill, "
...
{	//< CSD-030326
	char szQuerry[512];
	sprintf(szQuerry, "SELECT code, active, first, mark_num, make, info_num, name, degree0, degree1, degree2, degree3, degree4, nation FROM guild_list ORDER BY code");
...
{
	char szQuerry[512];
	sprintf(szQuerry, "SELECT * FROM ability_limit");
	HSTMT	hStmt = NULL;
	SQLAllocStmt(hDBC, &hStmt);
	RETCODE	retCode = SQLExecDirect(hStmt, (UCHAR*)szQuerry, SQL_NTS);
...
{
	char szQuerry[512];
	sprintf(szQuerry, "SELECT * FROM ability_divide");
	HSTMT	hStmt = NULL;
	SQLAllocStmt(hDBC, &hStmt);
	RETCODE	retCode = SQLExecDirect(hStmt, (UCHAR*)szQuerry, SQL_NTS);
...
bool LoadDualInfo()//»ñÈ¡×ªÉúÌõ¼þ
{	//< CSD-030522
	char szQuerry[512];
	sprintf(szQuerry, "SELECT * FROM dual_info ORDER BY step");
	HSTMT	hStmt = NULL;
	SQLAllocStmt(hDBC, &hStmt);
	RETCODE	retCode = SQLExecDirect(hStmt, (UCHAR*)szQuerry, SQL_NTS);
...
//	¸®ÅÏ°ª : SQL¹®À» ½ÇÇàÇßÀ»¶§ »ý±ä °á°ú°ª( ¼º°ø, ½ÇÆÐ )À» ¸®ÅÏÇØÁØ´Ù.
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
{	//< CSD-030517
	...
	for( int i=0; i< MAX_GAMBLE_ITEM_KIND; i++ )
	{
		...
		{
			sprintf(szQuerry, "house = %d AND type = %d", i, j );
			size = 0;
	...
	sprintf(szQuerry, "SELECT house, type, item_no from gamble_item" );
	SQLAllocStmt(hDBC, &hStmt);
	retCode = SQLExecDirect(hStmt, (UCHAR *)szQuerry, SQL_NTS);
...
int LoadNationItem( int week )
{
	...
	int row = 0;
	sprintf(szQuerry, "map = '%s' AND week = %d", MapName, week );
	GetRowLineOfSQL( "nation_item", "item_no", &row, szQuerry );
	if( row <= 0 ) return 0;
	
	sprintf(szQuerry, "SELECT item_no, money, fame FROM nation_item WHERE map = '%s' AND week = %d", MapName, week );
	SQLAllocStmt(hDBC, &hStmt);
	retCode = SQLExecDirect(hStmt, (UCHAR *)szQuerry, SQL_NTS);
...
int LoadGuildHouse()
{
	...
	sprintf(szQuerry, "SELECT id, lv, nation, blue, red, map_name, x, y, guild_code, event_no, date "
		"FROM guild_house" );
	SQLAllocStmt(hDBC, &hStmt);
	retCode = SQLExecDirect(hStmt, (UCHAR *)szQuerry, SQL_NTS);
	if( !SQLOK(retCode) ) 
...
void InitItemLog( int port )
{
	...
	{
		char qurry[MAX_PATH];
		sprintf( qurry, "DELETE log_item_event WHERE date < getdate()-%d", g_item_log.GetDeleteDay() );
...
{	//< CSD-030306
	...
	sprintf(szQuerry, "SELECT id FROM scenario_dungeon ORDER BY id DESC" );
	SQLAllocStmt(hDBC, &hStmt);
	retCode = SQLExecDirect(hStmt, (UCHAR *)szQuerry, SQL_NTS);
	...
	sprintf(szQuerry, "SELECT id, port1, port2 FROM scenario_dungeon");
	SQLAllocStmt(hDBC, &hStmt);
	retCode = SQLExecDirect(hStmt, (UCHAR *)szQuerry, SQL_NTS);
	if( !SQLOK(retCode) ) 
	{
...
int LoadDungeonMap()
{
	...
	sprintf( szQuerry, "map_name = '%s'", MapName );
	GetRowLineOfSQL( "scenario_dungeon_map", "*", &nRow, szQuerry );
	if( nRow <= 0 ) return 0;
	
	sprintf(szQuerry, "SELECT layer, trap_count, type, out_map, x, y FROM scenario_dungeon_map WHERE map_name = '%s'", MapName );
	SQLAllocStmt(hDBC, &hStmt);
	retCode = SQLExecDirect(hStmt, (UCHAR *)szQuerry, SQL_NTS);
	if( !SQLOK(retCode) ) 
	{
...
int LoadDungeonTrap( CDungeonObject *pData, int layer )
{
	...
	sprintf(szQuerry, "SELECT * FROM scenario_trap WHERE map_name = '%s' AND layer = %d ", MapName, layer );
	SQLAllocStmt(hDBC, &hStmt);
	retCode = SQLExecDirect(hStmt, (UCHAR *)szQuerry, SQL_NTS);
	if( !SQLOK(retCode) ) 
	{
	...
	while( SQLOK( retCode ) )
	{
		...
		CharUpper( map_name );
		strcpy( pData->m_szOutputMap, map_name );
...
int LoadDungeonMapEach( int port, int *temp )
{
	...
	sprintf(szQuerry, "SELECT id, continue_time, start_time, wait_time FROM scenario_dungeon WHERE port1 = %d", port );
	SQLAllocStmt(hDBC, &hStmt);
	retCode = SQLExecDirect(hStmt, (UCHAR *)szQuerry, SQL_NTS);
	if( !SQLOK(retCode) ) 
	{
...
int CEventMgr::LoadNpcItem()
{
	...
	int row = 0;
	sprintf( szQuerry, "map = '%s'", MapName );
	GetRowLineOfSQL( "Event_MonsterItem", "*", &row, szQuerry );
	if( row <= 0 ) return 0;
	
	m_eventNpcItem.Set( row );
	
	sprintf(szQuerry, "SELECT event_no, event_start_date, event_end_date, npc_no, [percent], "
...
int CEventMgr::LoadNpcCreate()
{
	...
	sprintf(szQuerry, "SELECT event_no, event_start_date, event_end_date, map_x, map_y, npc_no, "
...
int CEventMgr::LoadEventMap()
{
	...
	sprintf(szQuerry, "SELECT event_no, event_start_date, event_end_date, object_toi, object_b "
...
int CEventMgr::LoadEventSound()
{
	...
	int row = 0;
	sprintf( szQuerry, "[all] <> 0 OR map = '%s'", MapName );
	GetRowLineOfSQL( "Event_Sound", "*", &row, szQuerry );
	if( row <= 0 ) return 0;
	
	m_eventSound.Set( row );
	
	sprintf(szQuerry, "SELECT event_no, event_start_date, event_end_date, sound"
...
int CEventMgr::LoadEventMessage()
{
	...
	int row = 0;
	sprintf( szQuerry, "[all] <> 0 OR nation = %d OR map = '%s'", nation_num, MapName );
	GetRowLineOfSQL( "Event_Message", "*", &row, szQuerry );
	if( row <= 0 ) return 0;
	
	m_eventMessage.Set( row );
	
	sprintf(szQuerry, "SELECT event_no, event_start_date, event_end_date, [all], nation, [time], "
...
int CEventMgr::LoadEventNpcScript()
{
	...
	sprintf(szQuerry, "SELECT event_no, event_start_date, event_end_date, script_no, number, event_npcitem_bag, "
...
int CEventMgr::LoadItemBag()
{
	...
	sprintf(szQuerry, "SELECT * FROM Event_NpcItem_Bag" );
	SQLAllocStmt(hDBC, &hStmt);
	retCode = SQLExecDirect(hStmt, (UCHAR *)szQuerry, SQL_NTS);
	if( !SQLOK(retCode) )
	{
...
int CEventMgr::LoadEventMoveMap()
{
	...
	sprintf(szQuerry, "SELECT event_no, event_start_date, event_end_date, [all], sex, class, nation, "
...
int	LoadGuardianGuildList()
{
	...
	sprintf(szQuerry, "SELECT guild_code FROM guardian_list");
	SQLAllocStmt(hDBC, &hStmt);
	retCode = SQLExecDirect(hStmt, (UCHAR *)szQuerry, SQL_NTS);
...
{	//< CSD-030522
	char szQuerry[512];
	sprintf(szQuerry, "SELECT * FROM arena_info ORDER BY map_port");
	HSTMT	hStmt = NULL;
	SQLAllocStmt(hDBC, &hStmt);
...
{	//< CSD-030522
	...
	char szQuerry[512];
	sprintf(szQuerry, "SELECT * FROM arena_game_info WHERE map_port = %d", g_MapPort);
...
{	//< CSD-030522
	...
	for (CArenaManager::ITOR_GAME i = mpGame.begin(); i != mpGame.end(); ++i)
	{
		...
		char szQuerry[512];
		sprintf(szQuerry, "SELECT * FROM arena_team_info WHERE idGame = %d", nGame);
		HSTMT	hStmt = NULL;
		SQLAllocStmt(hDBC, &hStmt);
...
{	//< CSD-030522
	...
	for (CArenaManager::ITOR_GAME i = mpGame.begin(); i != mpGame.end(); ++i)
	{
		...
		char szQuerry[512];
		sprintf(szQuerry, "SELECT * FROM arena_bonus_info WHERE idGame = %d", nGame);
		HSTMT	hStmt = NULL;
		SQLAllocStmt(hDBC, &hStmt);
...
{   //< CSD-030522
	char szQuerry[512];
	sprintf(szQuerry, "SELECT * FROM hunt_info WHERE map_port = %d ORDER BY idHunt", g_MapPort);
...
{   //< CSD-030522
	...
	for (CRegenManager::ITOR_GROUP i = mpGroup.begin(); i != mpGroup.end(); ++i)
	{
		...
		char szQuerry[512];
		sprintf(szQuerry, "SELECT * FROM hunt_group_info WHERE idHunt = %d ORDER BY regen_order", i->first);
...
{   //< CSD-030522
	...
	for (CRegenManager::ITOR_GROUP i = mpGroup.begin(); i != mpGroup.end(); ++i)
	{
		...
		for (CGroupInfo::ITOR_REGEN j = ltRegen.begin(); j != ltRegen.end(); ++j)
		{
			...
			char szQuerry[512];
			sprintf(szQuerry, "SELECT * FROM hunt_monster_info WHERE idGroup = %d ORDER BY monster_type", pRegen->GetGroupNumber());
...
{   //< CSD-030522
	...
	for (CRegenManager::ITOR_GROUP i = mpGroup.begin(); i != mpGroup.end(); ++i)
	{
		...
		for (CGroupInfo::ITOR_REGEN j = ltRegen.begin(); j != ltRegen.end(); ++j)
		{
			...
			char szQuerry[512];
			sprintf(szQuerry, "SELECT * FROM hunt_place_info WHERE idGroup = %d ORDER BY number", pRegen->GetGroupNumber());
...
{   //< CSD-030804
	...
	char szQuerry[512];
	sprintf(szQuerry, "SELECT * FROM WarStartUpTBL WHERE Type = 2 ORDER BY DayofWeek");
...
int GetItemLimitCount_SQL( WORD port, DWORD *pLimit )
{
	...
	sprintf( szQuerry, "SELECT limit_number FROM ItemLimitMgrTable WHERE port = %d", port );
	*pLimit = 0;
...
bool LoadEventTreasureXY()
{
	int nRow = 0;
	char szQuerry[512];
	sprintf( szQuerry, "mapname = '%s'", MapName );
	...
	sprintf(szQuerry, "SELECT x, y FROM Event_treasure_xy WHERE mapname = '%s'", MapName );
	HSTMT hStmt = NULL;
	SQLAllocStmt(hDBC, &hStmt);
	RETCODE	retCode = SQLExecDirect(hStmt, (UCHAR*)szQuerry, SQL_NTS);	
	if (retCode != SQL_SUCCESS_WITH_INFO && retCode != SQL_SUCCESS) 
...
bool LoadEventTreasureGuard()
{
	...
	char szQuerry[512];
	sprintf(szQuerry, "SELECT lv, npc1, npc1_count, npc2, npc2_count, npc3, npc3_count, npc4, npc4_count, npc5, npc5_count FROM Event_treasure_Level" );
	HSTMT hStmt = NULL;
	SQLAllocStmt(hDBC, &hStmt);
	RETCODE	retCode = SQLExecDirect(hStmt, (UCHAR*)szQuerry, SQL_NTS);	
	if (retCode != SQL_SUCCESS_WITH_INFO && retCode != SQL_SUCCESS) 
