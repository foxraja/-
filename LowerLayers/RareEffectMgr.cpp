...
inline int	CRareEffectMgr::AddANDCheckStaticRareMaxValue(const int iKind,short &nNow, const int iAddValue,bool bSymbol)//soto-1.02
{
	...
	else
	{
		...
		iAdded	= nNow+iAddValue;//藤속돨令
		if( !iMax ) { return 0; }//離댕令槨0轟훨부묘콘.

		if( iMax <= iAdded )//흔벎藤속돨令댕黨離댕令，삔퓻齡긴냥離댕令
		{
			nNow = iMax;//맥스로 셋팅
...
//	case  RARE_VIGOR			:{AddANDCheckStaticRareMaxValue(iKind, SR.nVigor			, iValue);	}break;	
	...
	case  RARE_MAGIC_POWER		:{AddANDCheckStaticRareMaxValue(iKind, SR.nMagic_power		, iValue);	}break;	//침藤겟롸궐
	...
	case  RARE_NATURE_POWER		:{AddANDCheckStaticRareMaxValue(iKind, SR.nNature_power		, iValue);	}break;	//加藤겟롸궐
