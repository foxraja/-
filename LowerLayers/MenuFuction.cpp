...
int CMoneyToFame::LoadData()
{
	char file[MAX_PATH];
	FILE *fp;
	sprintf( file,"%s/data/money_fame.txt", GameServerDataPath );
	fp = fopen( file, "rt" );
	if( !fp ) return 0;
...
int InitDataFromNewDay( int day, int month )
{
	...
	if( g_MapPort == port )
	{
		CallClient( DB_DEMON, CMD_REFRESH_SALVATION_NAME );
		MyLog( 0, "Send to DB_DEMON for 'Delete Salvation'" );
	}
	else
	{
...
{
	strcpy( m_MapNation[m_Count].m_MapFile, map_name );
...
int LoadMapPotal()
{
	...
	sprintf( filename, "%s/data/map_potal.txt", GameServerDataPath );
	int nation;
	int map_number;
	int money, x, y, level;
