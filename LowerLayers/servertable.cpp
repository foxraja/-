...
CServerTable::~CServerTable()
{
	...
	SAFE_DELETE(this->m_pOwnServerData);
...
void CServerTable::NotifyServerStatus()
{
	...
	if(this->GetOwnServerData()->dwStatus == STATUS_FINISH_RELOAD_GAMESERVER_DATA)
	{
		pPacket->dwServerStatus = this->GetOwnServerData()->dwStatus;
		this->GetOwnServerData()->dwStatus = STATUS_ACTIVATED;
	}
	else
		pPacket->dwServerStatus = this->GetOwnServerData()->dwStatus;
	//
...
{	//< CSD-030821
	...
	if (!pServerData->dwConnectionIndex)
	{
		MyLog(LOG_FATAL, "CServerTable::Send - pServerData->dwConnectionIndex = %d", pServerData->dwConnectionIndex);
		return false;
	}
	
	return (m_pINet->SendToServer(pServerData->dwConnectionIndex, pMsg, dwLength, FLAG_SEND_NOT_ENCRYPTION)) ? true:false;
}	//> CSD-030821

bool CServerTable::Send( DWORD dwConnectionIndex, char* pMsg, DWORD dwLength )
{
	return (m_pINet->SendToServer(dwConnectionIndex, pMsg, dwLength, FLAG_SEND_NOT_ENCRYPTION)) ? true:false; // CSD-030314
}
...
//		_asm int 3;
	...
	return (m_pINet->SendToServer(pProxyServer->dwConnectionIndex, pMsg, dwLength, FLAG_SEND_NOT_ENCRYPTION)) ? true:false; // CSD-030314
}
...
bool CServerTable::SendToDBDemon( char *pMsg, DWORD dwLength )
{
	...
	for( pDBDemon = this->m_pServerListHead; pDBDemon; pDBDemon = pDBDemon->pNextServerData ) // 031021 kyo 
	{
...
DWORD CServerTable::SetNumOfUsers( DWORD dwNum , const int iCallPoint, const int iCallCn, const DWORD Flag,const char* szName)
{
